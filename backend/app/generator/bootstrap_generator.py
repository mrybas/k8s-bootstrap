...
class BootstrapGenerator:
    ...
    def __init__(self, cluster_name: str, repo_url: str, branch: str = "main", 
                 ...
                 skip_git_push: bool = False, cni_bootstrap_component: str = None,
                 dns_bootstrap_component: str = None,
                 cni_chaining_with_cilium: bool = False):
        ...
        # CNI to install before Flux (for clusters without CNI)
        self.cni_bootstrap_component = cni_bootstrap_component
        # DNS to install before Flux (for clusters without DNS)
        self.dns_bootstrap_component = dns_bootstrap_component
        # Install Cilium in chaining mode before Kube-OVN
        self.cni_chaining_with_cilium = cni_chaining_with_cilium
    ...
    def generate_bootstrap_script(self, repo_path: Path, categories: List[Dict[str, Any]] = None, 
                                    manifest_components: List[Dict[str, Any]] = None):
        """Generate bootstrap.sh script using template
        
        Note: manifest_components parameter is deprecated - manifest-based components
        now have Helm charts generated by ManifestChartGenerator and are installed via Flux.
        """
        context = {
            ...
            # CNI bootstrap for clusters without CNI (e.g., Talos with cni: none)
            "cni_bootstrap_component": self.cni_bootstrap_component,
            # Install Cilium in chaining mode before primary CNI
            "cni_chaining_with_cilium": self.cni_chaining_with_cilium,
            # DNS bootstrap for clusters without DNS
            "dns_bootstrap_component": self.dns_bootstrap_component,
        }
